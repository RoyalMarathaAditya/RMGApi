...
{
    /// <inheritdoc />
    public partial class FixBulkImportEmailMatching : Migration
    {
        ...
        {
            // Restore the previous version with EmployeeCode matching
            migrationBuilder.Sql($"DROP PROCEDURE IF EXISTS {SpName}");
            migrationBuilder.Sql($@"
                CREATE PROCEDURE {SpName}
                    @BatchId UNIQUEIDENTIFIER,
                ...
                BEGIN
                    ...
                    BEGIN TRY
                        ...
                        -- Step 1: Update existing employees matched by EmployeeCode
                        UPDATE e
                        SET 
                            e.FullName = s.FullName,
                            e.Email = s.Email,
                        ...
                        -- Step 2: Insert new employees not already in the Employees table
                        INSERT INTO Employees (
                            EmployeeCode, FullName, Email,
                            DOJ, LWD,
                            DesignationId,
                        ...
                        LEFT JOIN StatusMasters st ON st.Name = ISNULL(s.ActiveStatus, 'Active') AND st.IsDeleted = 0
                        WHERE s.BatchId = @BatchId
                          AND NOT EXISTS (SELECT 1 FROM Employees e WHERE e.EmployeeCode = s.EmployeeCode);

                        SET @InsertedRows = @@ROWCOUNT;
                        SET @ImportedRows = @UpdatedRows + @InsertedRows;

                        -- Step 3: Soft delete employees not present in the uploaded file (matched by EmployeeCode)
                        UPDATE e
                        SET 
                            e.IsDeleted = 1,
                            e.ModifiedOn = GETUTCDATE(),
                        ...
                        WHERE e.IsDeleted = 0
                          AND NOT EXISTS (
                            SELECT 1 FROM EmployeeImportStaging s 
                            WHERE s.BatchId = @BatchId AND s.EmployeeCode = e.EmployeeCode
                          );
